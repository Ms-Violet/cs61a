...
def balanced(m):
    ...
    if is_planet(m):
        ...
        l_end, r_end = end(left(m)), end(right(m))
        return total_weight(l_end) * length(left(m)) == total_weight(r_end) * length(right(m)) and balanced(
            l_end) and balanced(r_end)
...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        tree_branch = [totals_tree(end(left(m)))] + [totals_tree(end(right(m)))]
    return tree(tree_label, tree_branch)
