...
def swine_align(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 4a
    "*** YOUR CODE HERE ***"
    a, b = max(player_score, opponent_score), min(player_score, opponent_score)
    if a * b == 0:
        return False
    while b:
        a, b = b, a % b
...
def make_averaged(original_function, trials_count=1000):
    ...
    def averaged_dice(*arg):
        ...
        for i in range(trials_count):
            sum_dice += original_function(*arg)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    max_score, num_rolls, max_rolls = 0, 10, 0
    while num_rolls > 0:
        current_score = make_averaged(roll_dice, trials_count)(num_rolls, dice)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    if pig_pass(score, opponent_score) or swine_align(score, opponent_score) or free_bacon(opponent_score) >= cutoff:# Replace this statement
        num_rolls = 0
    return num_rolls
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    num_rolls += 1
    while num_rolls < 3:
        score += roll_dice(num_rolls, dice=six_sided)
        if score > 100:
            break
        num_rolls += 1
    return num_rolls  # Replace this statement
